        ...
        for det in detections:
            if det['confidence'] > 0.7:  # Lowered threshold
                ...
                if face.size > 0 and face.shape[0] >= 32 and face.shape[1] >= 32:
                    ...
                    print(f"Face too small: shape={face.shape}")
            else:
...
def train_model(num_identities=100):
    ...
    y_train = []
    failed_images = []
    total_images = sum(len(os.listdir(os.path.join(train_path, person)))
                       for person in os.listdir(train_path) if os.path.isdir(os.path.join(train_path, person)))
    processed = 0
    for person in os.listdir(train_path):
        person_dir = os.path.join(train_path, person)
        if os.path.isdir(person_dir):
    ...
    history = model.fit(X_tr, y_tr, epochs=50, batch_size=32, validation_data=(X_val, y_val), callbacks=[early_stopping])
...
def test_model(model, le):
    ...
    X_test = []
    y_test = []
    total_test_images = sum(len(os.listdir(os.path.join(test_path, person)))
                            for person in os.listdir(test_path) if os.path.isdir(os.path.join(test_path, person)))
    processed = 0
    skipped_labels = []
